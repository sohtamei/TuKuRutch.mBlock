...
//		var catName:String = Specs.categories[selectedCategory][1];
	...
	{
		if(Main.app.viewedObj() is ScratchSprite){
			return category;
		}
		switch(category){
	...
	static private function canShowInArduinoMode(spec:Array):Boolean
	{
		...
		if(Main.app.stageIsArduino && categoryId == Specs.controlCategory){
			switch(spec[3]){
				case "stopScripts":
				case "whenCloned":
				case "createCloneOf":
	...
	*/
	...
	private function addBlocksForExtension(ext:ScratchExtension):void {
		...
		var opPrefix:String = ext.useScratchPrimitives ? '' : 'robot.';//ext.name + '.';
