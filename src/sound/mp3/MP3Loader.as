...
public class MP3Loader {
	...
	public static function convertToScratchSound(sndName:String, sndData:ByteArray, whenDone:Function):void {
		...
		if (mp3Info.sampleCount == 0) { // bad MP3 data
			if (Main.app.lp) Main.app.removeLoadProgressBox();
			whenDone(null);
			return;
		}
		load(sndData, loaded);
	...
	private static function extractSamples(sndName:String, mp3Snd:Sound, mp3SampleCount:int, whenDone:Function):void {
		...
		function convertNextChunk():void {
			...
			if (count == 0) { // finished!
				if (Main.app.lp) Main.app.lp.setTitle('Compressing...');
			...
			if (Main.app.lp) Main.app.lp.setProgress(Math.min(convertedSamples / mp3SampleCount, 1));
